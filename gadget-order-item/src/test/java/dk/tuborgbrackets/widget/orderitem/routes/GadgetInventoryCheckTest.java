...
public class GadgetInventoryCheckTest extends CamelTestSupport {
	...
	protected RouteBuilder[] createRouteBuilders() throws Exception {
		return new RouteBuilder[] { new GadgetInventoryCheck(), new RouteBuilder() {
			public void configure() {
		        this.getContext().addComponent("gadgetOrderItem", queueComponent);
 			  			  
				from("direct:start")
				.setHeader("JMSCorrelationID",constant("foo-123"))
				.to("gadgetOrderItem:queue:NewOrderItem?useMessageIDAsCorrelationID=true").to("mock:result");
			}
		} };
	}
